...
<smf>';
	...
	<msgs><![CDATA[' . (!$context['browser']['is_ie'] && empty($shoutbox['showmsg_down']) ? '<thead id="shoutbox_msgs"></thead>' : '');
		...
		foreach ($context['shoutbox_echo']['msgs'] as $shout)
			echo '<tr id="shoutbox_row' . (empty($shoutbox['showmsg_down']) ? $_GET['row']-- : $_GET['row']++) . '">' . (empty($shoutbox['align_nicks']) ? ('<td class="' . $shoutbox['printClass'] . '" valign="top">' .  $shout['poster'] . $shout['message']) : ('<td nowrap="nowrap" style="text-align:right" class="' . $shoutbox['printClass'] . '" valign="top">' .  $shout['poster'] . '</td><td style="text-align:left" class="' . $shoutbox['printClass'] . '" valign="top">' . $shout['message'])) . '</td></tr>';
