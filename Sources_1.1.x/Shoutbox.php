...
{
	global $context, $txt, $scripturl, $shoutbox, $db_prefix, $ID_MEMBER;
	...
	if (isset($_GET['restart']) ||  empty($_SESSION['shoutbox_lastget']) || $_SESSION['shoutbox_lastget'] < time() - $shoutbox['refreshShouts'] + 15 || $_SESSION['shoutbox_lastget'] <= $shoutbox['lastPrune'])
	{
		...
		{
			$query_count = db_query("SELECT COUNT(*) FROM {$db_prefix}shoutbox", __FILE__, __LINE__);
			list ($limit_start) = mysql_fetch_row($query_count);
	...
	// so, no new msgs?
	if (empty($error) && mysql_num_rows($query) == 0)
		die;
	elseif (!empty($error))
	{
	...
	// last id
	$query_last = db_query("SELECT MAX(ID_SHOUT) FROM {$db_prefix}shoutbox", __FILE__, __LINE__);
	list ($_SESSION['shoutbox_lastid']) = mysql_fetch_row($query_last);
	mysql_free_result($query_last);
...
{
	global $func, $ID_MEMBER, $txt, $user_info;
	global $context, $db_prefix, $shoutbox;
	...
	$msg = $func['htmltrim']($msg);
	$msg = $func['htmlspecialchars']($msg, ENT_QUOTES);
	$msg = strip_tags($msg);
	$msg = addslashes($msg);
	...
	{
		db_query("TRUNCATE TABLE {$db_prefix}shoutbox", __FILE__, __LINE__);
	...
	// from here any error will be printed
 	if ($func['strlen']($msg) < $shoutbox['minMsgLenght'])
		return Shoutbox_GetMsgs($txt['sb_12'] . $shoutbox['minMsgLenght']);
	if ($func['strlen']($msg) > $shoutbox['maxMsgLenght'])
	...
	if (strstr($msg, 'http://'))
	{
		...
		foreach (explode(' ', $msg) as $w)
			if (substr($w, 0, 7) == 'http://' && $func['strlen']($w) > 7)
				$links[] = '<a href="' . $w . '" target="_blank">' . ($func['strlen']($w) > $shoutbox['maxLinkLenght'] ? substr($w, 0, $shoutbox['maxLinkLenght'] - 8) . '...' . substr($w, -8) : $w) . '</a>';
			else
				$links[] = $w;
		$msg = implode(' ', $links);
	}
	...
	$style = $func['htmltrim']($style);
	$style = $func['htmlspecialchars']($style, ENT_QUOTES);
	...
	// color gruop
	$query =  db_query("
			SELECT mg.onlineColor
			FROM {$db_prefix}membergroups AS mg
				LEFT JOIN {$db_prefix}members AS mem ON (mem.ID_MEMBER = $ID_MEMBER)
			WHERE mg.ID_GROUP = IF(mem.ID_GROUP = 0, mem.ID_POST_GROUP, mem.ID_GROUP)
			LIMIT 1", __FILE__, __LINE__);
	@list($color) = mysql_fetch_row($query);
	mysql_free_result($query);
...
{
	global $context, $txt, $scripturl, $settings, $shoutbox, $db_prefix;
	...
	if ($limit == 0)
	{
		...
		$query = db_query("
				SELECT MAX(ID_SHOUT) FROM {$db_prefix}shoutbox", __FILE__, __LINE__);
		@list ($limit) = mysql_fetch_row($query);
	...
	$query = db_query("
				SELECT COUNT(*) FROM {$db_prefix}shoutbox" . ($limit > 0 ? "
					WHERE ID_SHOUT <= $limit" : ""), __FILE__, __LINE__);
...
{
	global $func, $context, $txt, $shoutbox, $db_prefix, $ID_MEMBER;
	...
	if (isset($_GET['delete']))
	{
		...
		db_query("DELETE FROM {$db_prefix}shoutbox WHERE ID_SHOUT = $shout LIMIT 1", __FILE__, __LINE__);
...
{
	global $context, $db_prefix, $txt, $scripturl, $func;
	...
	elseif (isset($_POST['user']) && checkSession('get', '', false) == '')
	{
		...
		$reason = empty($_POST['reason']) ? '' : addslashes(strip_tags($func['htmlspecialchars']($func['htmltrim'](stripslashes($_POST['reason'])))));
		$days = empty($_POST['days']) ? 0 : ((int) $_POST['days']) * 86400;

		db_query("
			UPDATE {$db_prefix}shoutbox_ban
			SET
				banEnd = " . ($days == 0 ? "0" : "banStart + $days") . ",
				reason = '$reason'
			WHERE ID_MEMBER = $u LIMIT 1", __FILE__, __LINE__);
	...
	$query = db_query("
				SELECT
					...
					IFNULL(mem.realName, '') as realName
...
{
	global $context, $db_prefix, $txt, $scripturl, $func, $ID_MEMBER, $user_info;
	...
	if (isset($_POST['users']))
	{
		...
		if (trim($_POST['users']) != '')
		{
			$users_string = strtr(addslashes($func['htmlspecialchars'](stripslashes($_POST['users']), ENT_QUOTES)), array('&quot;' => '"'));
			...
			if (!empty($users))
			{
				$request = db_query("SELECT ID_MEMBER FROM {$db_prefix}members
					WHERE memberName IN ('" . implode("','", $users) . "') 
						OR realName IN ('" . implode("','", $users) . "')
		...
		foreach ($usersIDs as $u)
			$set .= "
						($u, $start, $days, '$reason', '$user_info[name]', $ID_MEMBER),";

		db_query("
				REPLACE INTO {$db_prefix}shoutbox_ban
					(ID_MEMBER, banStart, banEnd, reason, banBy, banByID)
				VALUES" . substr($set, 0, -1), __FILE__, __LINE__);
